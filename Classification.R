...
table(y=spamfit$y, yhat=yhat_train)# training set confusion matrix, off-diagonal: false positive / false negative

# exercise: how well does the model predict on the data in spamtest.csv?  
(597+198)/(nrow(spamfit))

# forcast test data and check the out-sample accurancy.
#import testing dataset
spamtest$lm1_forcast = predict(lm1, spamtest)
yhat_test = ifelse(spamtest$lm1_forcast >= 0.5, 1, 0)
