...
ggplot(mpg, aes(displ, hwy)) +
  geom_point(aes(color = class)) +
  geom_smooth(se = FALSE) + #change to True, shows CI
  labs(title = "Fuel efficiency generally decreases with engine size")
...
g = ggplot(mpg, aes(x=cty)) # save plot in g
g + geom_density(aes(fill=factor(cyl))) # fill works like color
...
ggplot(mpg, aes(x=model, y=hwy)) + 
  geom_bar(stat='identity')  #adds up all hwy for the same model

# a little easier to read
ggplot(mpg, aes(x=model, y=hwy)) + 
  geom_bar(stat='identity') + 
  coord_flip()  # flip from horizon to vertical

# What's wrong?
mpg

# Let's calculate average MPG across model years
# We do this using pipes: （create a new dataset saving in mpg_summ)
mpg_summ = mpg %>%
